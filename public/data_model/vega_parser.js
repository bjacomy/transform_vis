...
import { Transform, TransformLite } from '../lib/Transform';
import schemaParser from 'Transform-schema-url-parser';
...
const DEFAULT_SCHEMA = 'https://Transform.github.io/schema/Transform/v5.json';
...
export class TransformParser {
  ...
  async _parseAsync() {
    if (this.isTransformLite !== undefined) throw new Error();
    ...
    if (!_.isPlainObject(this.spec)) {
      throw new Error(
        i18n.translate('visTypeTransform.TransformParser.invalidTransformSpecErrorMessage', {
          defaultMessage: 'Invalid Transform specification',
        })
      );
    }
    this.isTransformLite = this._parseSchema();
    this.useHover = !this.isTransformLite;
    ...
    if (this.useMap) {
      ...
      // Default autosize should be fit, unless it's a map (leaflet-Transform handles that)
      this.spec.autosize = { type: 'fit', contains: 'padding' };
    }

    await this._resolveDataUrls();

    if (this.isTransformLite) {
      this._compileTransformLite();
    }
  ...
  /**
   * Convert TransformLite to Transform spec
   * @private
   */
  _compileTransformLite() {
    this.vlspec = this.spec;
    // eslint-disable-next-line import/namespace
    const logger = Transform.logger(Transform.Warn); // note: eslint has a false positive here
    logger.warn = this._onWarning.bind(this);
    this.spec = TransformLite.compile(this.vlspec, logger).spec;

    // When using VL with the type=map and user did not provid their own projection settings,
    // remove the default projection that was generated by TransformLite compiler.
    // This way we let leaflet-Transform library inject a different default projection for tile maps.
    // Also, VL injects default padding and autosize values, but neither should be set for Transform-leaflet.
    if (this.useMap) {
      ...
      if (this.vlspec.config === undefined || (hasConfig && !this.vlspec.config.projection)) {
        ...
        if (
          ...
          throw new Error(
            i18n.translate(
              'visTypeTransform.TransformParser.VLCompilerShouldHaveGeneratedSingleProtectionObjectErrorMessage',
              {
                defaultMessage:
                  'Internal error: Transform-Lite compiler should have generated a single projection object',
              }
            )
          );
        }
  ...
  _calcSizing() {
    ...
    if (!this.useMap) {
      // when useResize is true, Transform's canvas size will be set based on the size of the container,
    ...
    if (this.useResize && (this.spec.width || this.spec.height)) {
      if (this.isTransformLite) {
        ...
        this._onWarning(
          i18n.translate(
            'visTypeTransform.TransformParser.widthAndHeightParamsAreIgnoredWithAutosizeFitWarningMessage',
            {
              defaultMessage:
  ...
  _parseControlPlacement() {
    ...
    if (this.containerDir === undefined) {
      if (this._config.controlsLocation === undefined) {
        ...
        throw new Error(
          i18n.translate('visTypeTransform.TransformParser.unrecognizedControlsLocationValueErrorMessage', {
            defaultMessage:
    ...
    if (dir !== undefined && dir !== 'horizontal' && dir !== 'vertical') {
      throw new Error(
        i18n.translate('visTypeTransform.TransformParser.unrecognizedDirValueErrorMessage', {
  ...
  /**
   * Parse {config: kibana: {...}} portion of the Transform spec (or root-level _hostConfig for backward compat)
  ...
  _parseConfig() {
    ...
    if (this.spec._hostConfig !== undefined) {
      ...
      if (!_.isPlainObject(result)) {
        throw new Error(
          i18n.translate('visTypeTransform.TransformParser.hostConfigValueTypeErrorMessage', {
      ...
      this._onWarning(
        i18n.translate('visTypeTransform.TransformParser.hostConfigIsDeprecatedWarningMessage', {
          defaultMessage:
    ...
    if (_.isPlainObject(this.spec.config) && this.spec.config.kibana !== undefined) {
      ...
      if (!_.isPlainObject(result)) {
        throw new Error(
          i18n.translate('visTypeTransform.TransformParser.kibanaConfigValueTypeErrorMessage', {
            defaultMessage: 'If present, {configName} must be an object',
  ...
  _parseTooltips() {
    ...
    if (result.position === undefined) {
      ...
      throw new Error(
        i18n.translate(
          'visTypeTransform.TransformParser.unexpectedValueForPositionConfigurationErrorMessage',
          {
    ...
    if (result.padding === undefined) {
      ...
      throw new Error(
        i18n.translate('visTypeTransform.TransformParser.paddingConfigValueTypeErrorMessage', {
    ...
    if (result.centerOnMark === undefined) {
      ...
      throw new Error(
        i18n.translate('visTypeTransform.TransformParser.centerOnMarkConfigValueTypeErrorMessage', {
  ...
  _parseMapConfig() {
    ...
    const validate = (name, isZoom) => {
      ...
      if (val !== undefined) {
        ...
        this._onWarning(
          i18n.translate('visTypeTransform.TransformParser.someKibanaConfigurationIsNoValidWarningMessage', {
    ...
    if (res.mapStyle !== `default` && res.mapStyle !== false) {
      this._onWarning(
        i18n.translate('visTypeTransform.TransformParser.mapStyleValueTypeWarningMessage', {
          defaultMessage:
    ...
    if (maxBounds !== undefined) {
      if (
        ...
        this._onWarning(
          i18n.translate('visTypeTransform.TransformParser.maxBoundsValueTypeWarningMessage', {
  ...
  _parseBool(paramName, dstObj, dflt) {
    ...
    if (val === undefined) {
      ...
      this._onWarning(
        i18n.translate('visTypeTransform.TransformParser.someKibanaParamValueTypeWarningMessage', {
  ...
  /**
   * Parse Transform schema element
   * @returns {boolean} is this a TransformLite schema?
   * @private
   */
  _parseSchema() {
    if (!this.spec.$schema) {
      this._onWarning(
        i18n.translate('visTypeTransform.TransformParser.inputSpecDoesNotSpecifySchemaWarningMessage', {
          defaultMessage:
    ...
    const isTransformLite = schema.library === 'Transform-lite';
    const libVersion = isTransformLite ? TransformLite.version : Transform.version;

    if (versionCompare(schema.version, libVersion) > 0) {
      this._onWarning(
        i18n.translate('visTypeTransform.TransformParser.notValidLibraryVersionForInputSpecWarningMessage', {
          defaultMessage:
  ...
  async _resolveDataUrls() {
    ...
    this._findObjectDataUrls(this.spec, (obj) => {
      ...
      if (parser === undefined) {
        throw new Error(
          i18n.translate('visTypeTransform.TransformParser.notSupportedUrlTypeErrorMessage', {
    ...
    if (Array.isArray(obj)) {
      ...
      if (key === 'data' && _.isPlainObject(obj.url)) {
        ...
        if (obj.values !== undefined || obj.source !== undefined) {
          throw new Error(
            i18n.translate(
              'visTypeTransform.TransformParser.dataExceedsSomeParamsUseTimesLimitErrorMessage',
              {
                defaultMessage:
  ...
  _setDefaultColors() {
    ...
    if (this.isTransformLite) {
      // Transform-Lite: set default color, works for fill and strike --  config: { mark:  { color: '#54B399' }}
      this._setDefaultValue(defaultColor, 'config', 'mark', 'color');
    } else {
      // Transform - global mark has very strange behavior, must customize each mark type individually
      // https://github.com/Transform/Transform/issues/1083
