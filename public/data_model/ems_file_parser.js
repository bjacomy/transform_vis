...
export class EmsFileParser {
  ...
  parseUrl(obj, url) {
    if (typeof url.name !== 'string') {
      throw new Error(
        i18n.translate('visTypeTransform.emsFileParser.missingNameOfFileErrorMessage', {
          defaultMessage:
  ...
  async populateData(requests) {
    ...
    for (const { obj, name } of requests) {
      ...
      if (!foundLayer) {
        throw new Error(
          i18n.translate('visTypeTransform.emsFileParser.emsFileNameDoesNotExistErrorMessage', {
